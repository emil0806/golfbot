...
def find_best_ball(ball_positions, robot_position, front_marker):
    ...
    if previous_best_ball:
        ...
        print(
            f"Previous best ball: {previous_best_ball}, Distance: {old_dist:.2f}")
...
def is_edge_ball(ball, margin=150):
    ...
    near_right = 1600 - margin < x < 1600 + \
        margin and 50 + margin < y < 1050 - margin
    near_top = 50 - margin < y < 50 + margin and 250 + margin < x < 1600 - margin
    near_bottom = 1050 - margin < y < 1050 + \
        margin and 250 + margin < x < 1600 - margin
...
# Vi skal stoppe før vi når vi rammer ind i barrieren afhængig af pixels
